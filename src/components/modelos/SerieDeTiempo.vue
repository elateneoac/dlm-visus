<template>
    ...
    <div class="container my-3">
        ...
        <div class="row mb-3 justify-content-left">
            ...
            <div v-if="!this.grupo" class="col-2">
                <div class="input-group input-group-sm mb-3">
        ...
        <div class="my-3"></div>
        <EtiquetasControl v-if="this.hay_grupos" v-model="etiquetas_parametros"/>
    </div>
</template>
...
export default {
    ...
    props : ['df', 'x', 'y', 'grupo', 'margen', 'ancho', 'alto','estetica'],
    ...
    computed: {
        grupos() {
            return [ ...new Set(this.df.map(f => f[this.grupo]).filter(f => f !== 'casarosada' && this.etiquetas_parametros.estetica[f].mostrar) ) ];
        },
        hay_grupos() {
            return this.grupo && this.grupo.length;
        }
    },
    ...
    methods: {
        ...
        y_freqs() {
            ...
            .range([this.alto - this.margen.piso, this.margen.techo]);
        },
        ...
        curva(x, y) {
            ...
            return d3.line().curve(tipo).x(d => x(d[this.x])).y(d => y(d[this.y]))
        },
        ...
        dibujar() {
            ...
            if (this.hay_grupos) {
                for(let grupo of this.grupos) {
                    ...
                    .attr('d', punto(this.df.filter(d => d[this.grupo] == grupo && this.etiquetas_parametros.estetica[d[this.grupo]].mostrar).sort((a, b) => a[this.x] > b[this.x] ? 1 : -1)));
                }
